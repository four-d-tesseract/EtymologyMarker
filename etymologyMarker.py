...
def removeAffixes(formattedList, allWords):
    ...
    for x, word in enumerate(formattedList):
        if getOrigin(formattedList[x], allWords) == None:
            ...
            if allWords.get(re.sub('ly$', "le", formattedList[x])) != None: # Replaces -ly with -le if that produces a word.
                formattedList[x] = re.sub('ly$', "le", formattedList[x])    # Otherwise, strips off -ly.
            else:
                formattedList[x] = re.sub('ly$', "",   formattedList[x])
            if allWords.get(re.sub('$',"e", formattedList[x])) != None:    # Adds -e back onto words that had -e removed.
                formattedList[x] = re.sub('$',  "e", formattedList[x])
            if getOrigin(formattedList[x], allWords) == None:                     # Handles words that end in -y, which gets changed to -ie when a suffix is added.
...
def main():
    ...
    # Sorts dictionary as a list based on Word Count and writes %age composition.
